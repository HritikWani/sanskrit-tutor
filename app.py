...
def add_test():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            upload_result = cloudinary.uploader.upload(file, folder="tests", resource_type="auto")
...
def student_upload_answer():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename) and file.filename.lower().endswith('.pdf'):
            try:
                upload_result = cloudinary.uploader.upload(file, folder="answers", resource_type="auto")
                file_url = upload_result['secure_url']
